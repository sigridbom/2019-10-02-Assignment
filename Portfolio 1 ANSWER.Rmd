...
ballon_graph + geom_histogram(aes(y = ..density..), binwidth = 0.6) +
  ...
  labs(x = "Blowing up a ballon", y = "Density")
...
balloon_balance_graph <- 
#balloon balance numeric
```
...
The first graph shows a scatterplot of the data with a regression line (the red line). It looks like there could be two different groups in the data since there is two slight curves, as in bimodality (probably male and female)

The second graph shows the same, but the regression is now linear. This shows a positive relation between shoe size (which we can refer to body size) and breath hold. 

The third graph (bonus question) shows the correlation between shoe size and breath hold for men and women respectively. Here there is still a clear positive relation between shoe size and breath hold for women, but the regression for men look more constant. 
