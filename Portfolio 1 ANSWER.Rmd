...
I can see that the standard deviation is quite high for all three groups, so the mean does not represent the data as sample from a population very well.  ## OR DOES IT??

+ right and left in SD and SE
...
NOGET OM 2SEKURTOSIS OG 2SESKEWNESS HVIS TALLENE ER STØRRE END 1 ER DER SIGNIFIKANT FORSKEL PÅ DATASÆTTER OG ET NORMAL DISTRIBUERET DATASÆT CLOSE ENOUGH 

The kurtosis and skewness of a standard normal distribution is 0. But you would never find that in the real world, which is why a skewness between -1 and 1 is also acceptable for a normally distributed sample. 
...
The skewness is 0,58, which is in the interval for a normal distributed sample. The the skewness suggest a possible normal distribution is not enough when compared to the other parameters of the data. 
