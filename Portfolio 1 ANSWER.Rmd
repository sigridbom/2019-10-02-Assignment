...
The group of people with right ocular dominance also have the smallest errorbar, which means that it is the most likely to be a good representation as a sample from a population.
...
A very good reason for the data not being normally distributed is that the normal distribution allows negative values and extends to very large values witch this data set does not, because time cannot be measured negatively. 
...
The histogram and the Q-Q plot depict that data as probably not normally distributed. The histogram is positively skewed and the Q-Q plot show that the data are not looking normal because they do not follow the linear green line.  
...
I can conclude that there in general is a positive relation between shoe size (and there by body size) and the ability to hold ones breath (the larger the shoe size, the longer the breath hold). 
